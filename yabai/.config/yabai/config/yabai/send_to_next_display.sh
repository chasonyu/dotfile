...
send_window=${1:-}
all_display_indices_array=($(/usr/local/bin/yabai -m query --displays | /usr/local/bin/jq '.[].index | @sh' | tr -d \'\" | sort))
...
current_display_index=$(/usr/local/bin/yabai -m query --displays --display | /usr/local/bin/jq '.index')
...
if [[ -n ${next_display_index:+x} ]]; then
  ...
  if [ -n "$send_window" ]; then
    /usr/local/bin/yabai -m window --display $index
  fi
  /usr/local/bin/yabai -m display --focus $index
fi
